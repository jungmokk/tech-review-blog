...
def conduct_10_source_deep_research(device_obj: dict) -> dict:
    ...
    # 1. [원칙 필수] 동일 제품 전문 테크 유튜버 최소 5개 이상 영상 트랜스크립트 추출 및 분석
    from ai.youtube_transcript_extractor import search_and_extract_5_youtube_transcripts
    yt_transcripts = search_and_extract_5_youtube_transcripts(dev_name, eng_name)

    for yt in yt_transcripts:
        ch = yt.get("channel", "테크 유튜버")
        t = yt.get("title", "")
        summary = yt.get("summary", "")
        sub_status = "자막 트랜스크립트 전문 분석" if yt.get("has_subtitles") else "실사용 리뷰 분석"
        
        all_references.append({
            "source_no": len(all_references) + 1,
            "type": "YouTube Creator Hands-on",
            "name": f"유튜브 [{ch}]: {t[:45]}",
            "insight": f"({sub_status}) {summary[:180]}"
        })

    # 2. ZOL (中关村在线) 공인 하드웨어 지수 및 벤치마크
    ...
    # 3. GSMArena / PhoneArena 실측 배터리 및 카메라 랩 테스트
    all_references.append({
        "source_no": len(all_references) + 1,
    ...
    # 4. The Verge / Android Authority 사용성 종합 평가
    all_references.append({
        "source_no": len(all_references) + 1,
    ...
    # 5. 실사용자 커뮤니티 (Reddit / 클리앙 / 뽐뿌) 초기 피드백
    all_references.append({
        "source_no": len(all_references) + 1,
    ...
    web_refs = search_web_multi_sources(f"{eng_name} review hands-on specs", count=3)
    if not web_refs:
        web_refs = [
    ...
    print(f"✅ [Deep Research Complete] 총 {len(all_references)}개 다각도 출처 (유튜브 트랜스크립트 {len(yt_transcripts)}개 포함) 수집 및 종합 정리 완료!\n")
